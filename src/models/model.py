...
class Classifier(LightningModule):
    def __init__(self, cfg: DOGCATConfig):
        ...
        for idx, conv_layer in enumerate(cfg.conv_layers):
            ...
            h = compute_conv_dim(
                h, cfg.maxpool.kernel_size, cfg.maxpool.padding, cfg.maxpool.stride
            )
            w = compute_conv_dim(
                w, cfg.maxpool.kernel_size, cfg.maxpool.padding, cfg.maxpool.stride
            )
            hw.append([h, w])

        self.l1_in_features = (
            self.cfg.conv_layers[-1].out_channels * hw[-1][0] * hw[-1][1]
        )

        self.maxpool = nn.MaxPool2d(
            self.cfg.maxpool.kernel_size,
